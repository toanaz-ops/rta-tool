...
2. **L4a's wrong sign came from an estimator, not from a convention — and specifically from the estimator this repo does NOT ship** (probe §5, §8, corrected at PR #3 head `cdd8a25` after its own verifier refuted the first attribution). The rule that reproduces it is L4a **decision 6b's un-whitened** `ρ = |peak|/√(E₁E₂)` and the sign at that peak (`docs/dsp/2026-08-30-sweep-ir-l4a.md:1181` on `main`) — **planned, not shipped**: `relativePolarity()` exists nowhere in `core/` or `app/` today, which is why **Task E is where it lands**. *(Line note: PR #3 cites that rule as `:1195` because its own branch adds a warning block to the same L4a record and shifts the file by 24 lines; from `main` and from this branch the ρ definition is at **`:1181`**. Cite `:1181` in code comments, and `:1195` only when quoting PR #3.)* Applied across two systems with *different passbands*, two band-pass boxes spread the HP−LP offset by **62.9°** across the overlap instead of holding it constant, stretching the correlation envelope until a neighbouring, oppositely-signed lobe outgrows lag 0 (peak at −3.08 ms, 1.45× the lag-0 value) — while the **order-4** value at lag 0 stays positive in all five geometries, exactly as the identity says. At order 8 the peak moves further (−6.33 ms) onto a positive lobe and the offset spread reaches 86.3°, so the "right" order-8 reading is two errors landing on the same answer; the lag-0 claim is scoped to order 4 and no wider.
3. **The shipped correlator does NOT reproduce L4a — it fails differently.** `findDelayPhat`'s peak sign (`core/src/dsp/DelayFinder.cpp:34`, off a **PHAT-whitened** cross-spectrum) reads `+1` at order 4 and `−1` at order 8 on the identical pair — the **mirror** of the un-whitened reading. Whitening moved the failure from order 4 to order 8; it did not remove it, and neither rule reports that it has stopped tracking lag 0. **Two correlators over one unchanged pair of loudspeakers, disagreeing about polarity at two of six orders**, is the argument (probe §5).
4. **No sign convention in this engine can reach the order-4 reading.** Conjugating the transfer function negates the offset, and −0° = 0° and −180° = 180°, so an even-order pair reads identically either way (probe §6).

**What that binds on this lane.** (a) The wizard **never reads a topology sign off ANY correlation peak, whitened or not** — §4's complex band fit with its bounded `R` is the estimator, and `R` collapsing is precisely how it reports "these two are not a matched pair". (b) An even-order sign discrepancy is evidence about the **system** (a band-passed source, a processor whose HP is `(−s)^N/D`, or a wiring inversion the operator must be asked about under question (c)) — **never** evidence about a convention.

**Where the ruling is consumed.** Four places, each with a red-able fixture, each citing the probe doc: **D5** (the shipped signed lookup), **H9** (the wizard's comparison of `φ₀` against that lookup), **I1b** (the surface's target line), and **E8/E9** — the two documented failures of ρ, one of which (E9) rebuilds the exact band-pass geometry that produced the L4a reading, so the rule that caused the confusion is locked as a known failure the moment it is written down. The earlier draft of this plan claimed the seam was confined to D5; it was not (verifier defect 7) — H and I consume `expectedOffset` too, and now each pins a sign. Since PR #3 is **not merged**, a builder who reaches D5 before it lands cites the doc by branch and says so in the commit message; nothing here waits, because the ruling keeps the table as written.
...
- **ALIGN-R12 — two tolerance reconciliations, one of them a follow-up on shipped code, not a plan change.** *(a)* **Record §10.4's `R = 1 − 1e-12` is not simultaneously satisfiable with its own τ tolerance.** `R` is evaluated at `τ*`, and over a **discrete** bin grid `1 − R ≃ (πΔ·dτ)²(N²−1)/6`. Over the record's own 40–160 Hz window at 1 Hz bins (N = 121), `dτ = tauGridSeconds/20 = 5e-8 s` gives `1 − R = 6.0205e-11`; `1e-12` needs `dτ ≤ 6.50e-9 s = grid/154`. *Decision:* C1 asserts the **derived** floor `R ≥ 1 − 1.01·(πΔf·dτ_max)²/6` so the two tolerances are one statement. The record's `1e-12` should be re-stated as a function of the refinement bound, or its τ bound tightened to `grid/154`. *(b)* **The biquad two-spelling lock stays at 1e-12, and a shipped test is looser than the record without saying why.** Record §5 holds **1e-12** for `20log₁₀|H| == −attenuationDb` and this plan's **A6 holds 1e-12** — neither moves. But the already-shipped `core/tests/test_biquad_response.cpp:78` asserts `WithinAbs(rhs, 1e-9)`, and has since Wave 0's `b2172b3`, with **no justification recorded** for the three orders of magnitude. That is a **follow-up on the shipped test** (either justify 1e-9 or tighten it to the record's 1e-12), not a change to this plan; A6 must not be relaxed to match it.
- **ALIGN-R13 — record §3's `(−s)^N/D` explanation of the order-4 reading is arithmetically empty.** `(−s)^N = (−1)^N s^N`, so at every **even** N it is the identity map and cannot change an order-4 sign. PR #3 supplies the real mechanism (L4a decision 6b's **un-whitened** peak-sign rule, `docs/dsp/2026-08-30-sweep-ir-l4a.md:1181` on `main`, across two different passbands; probe §5 at head `cdd8a25`). *Decision:* the sentence is struck from the record at the orchestrator's amendment pass; this plan does not use it as evidence, and Task D's mutation is PR #3's `√2` red instead.
...
|---|---|---|
| C1 | exact case (§10.4), **tolerances derived from each other, on the DISCRETE bin grid** | **Fixture, fully declared** (ALIGN-R5 makes `binWidthHz` a required input, so it is part of the fixture, not an afterthought): `H_A ≡ 1`, `H_B = e^{j(φ₀ − 2πf τ₀)}`, τ₀ = +3.7 ms, φ₀ = π, unit coherence, **`binWidthHz = Δ = 1.0 Hz`**, window 40–160 Hz **inclusive** ⇒ **`N = 121` gated bins**. `R` is evaluated at `τ*`, **not** at `τ₀`, so it is a function of the refinement error `dτ` and the two assertions must be one statement. The fit sums over **bins, not a continuum**, so the exact form is the Dirichlet kernel<br>&nbsp;&nbsp;&nbsp;&nbsp;`R(dτ) = \|sin(πNΔ·dτ) / (N·sin(πΔ·dτ))\|`,&nbsp;&nbsp;small-argument&nbsp;&nbsp;**`1 − R ≃ (πΔ·dτ)²·(N²−1)/6`**.<br>**Arithmetic for this fixture**, `dτ_max = tauGridSeconds/20 = 5e-8 s`: `(π·1·5e-8)² = 2.467401e-14`; `× (121²−1)/6 = × 2440` → **`6.020459e-11`**, and the exact Dirichlet value is **`6.020462e-11`**. Assert `|tauSeconds − τ₀| ≤ dτ_max` **and** `agreement ≥ 1 − 1.001·6.0205e-11 = 1 − 6.0265e-11` (0.1% absorbs a ±1-bin window-edge convention; the expansion's own truncation is `O((πNΔdτ)⁴) ≈ 1e-21`). `|interceptRadians − π| ≤ 1e-9`. **Both residuals printed.**<br>**Why the previous draft went red** (round-2 verifier defect 3): it used the continuum form over `Δf = 120 Hz`, `(π·120·dτ)²/6 = 5.9218e-11`, which is **tighter than the truth** — the discrete sum spans `(N−1)Δ` but carries `N` terms, and the exact ratio of discrete to continuum is **`(N+1)/(N−1)`** = 1.016667 at 1 Hz bins and 1.033333 at 2 Hz bins (N = 61). (The verifier's `(N/(N−1))²` is a very close approximation of the same factor — 1.016736 and 1.033611 — and rounds to the two figures quoted; the plan states the exact form.) At **2 Hz bins** the same arithmetic gives `1 − R = 6.119e-11`, so a builder who changes `binWidthHz` **must recompute the floor from the formula**, not reuse the number. The record's `R = 1 − 1e-12` needs `dτ ≤ grid/154` — see ALIGN-R12 |
...
| D6 | core stays topology-free | **AMENDED 2026-09-16 by the Wave 3a build — the word-grep this row originally specified is FALSE on the tree, both before and after the lane.** It read: `grep -rniE "linkwitz|butterworth|crossover ?family|topolog" core/include core/src` returns only the pre-existing `ButterworthDesign` hits. On the UNTOUCHED tree at `a2de06e` it already also matches `BandWeights.h:41` (an IEC conformance note), `EqGainSolve.cpp:20` (a comment), `Decay.cpp:8` and `:86` (an include of `ButterworthDesign.h`) and four lines in `FilterBank.cpp` — prose and includes, not topologies. It is verifier defect 5 in this plan’s own acceptance row. Wave 3a then added a ninth hit, `CrossoverFit.h:78`, a doc comment reading “the topology the operator NAMED”, which is a signpost to the layer that owns the table and not core knowing what a topology is. **Shipped instead:** a DECLARATION-shaped scan run from the test (`app/tests/test_crossover_topology.cpp`) over `core/include` and `core/src` with comment-only lines stripped, forbidding `crossoverfamily`, `linkwitz` and `topolog` as identifiers. `butterworth` is deliberately NOT forbidden — `ButterworthDesign` is a filter DESIGNER and a polynomial is not a crossover topology. It runs in CI on all three operating systems instead of living in a grep someone has to remember to type; `rtatool_analysis_tests` gained `RTA_REPO_ROOT` for it. Record §9 is unchanged — only this row’s mechanism was wrong. |
...
| E6 | **no verdict ships** | the grep must match a **declaration**, never the comment that explains why there is none — the earlier word-grep went red against this plan's own prescribed header (verifier defect 1). Use (GNU grep) `grep -nE '^[[:space:]]*[^/*]*(is|has)?[A-Za-z_]*([Tt]hreshold|[Vv]erdict|[Aa]ccept)[A-Za-z_]*[[:space:]]*[(;=]' core/include/rta/ir/RelativePolarity.h` → **no match**. Three things earn their place: `[^/*]*` after the indent excludes `///` and ` *` comment lines; the trailing class is **`[(;=]`**, so a **function-shaped** declaration (`Verdict verdict() const;` — the spelling this plan itself uses at D5 and H8) is caught, which the earlier `(=|;)`-only form missed (round-2 verifier); and `(is|has)?` catches `hasVerdict()`. **Validated on this tree**: it matches `DelayPolicy.h:64` (`DelayVerdict verdict = …`) and `:75` (`double acceptMultiple = 4.0;`) and a synthetic `Verdict verdict() const;`, and **misses** `Polarity.h` and every `///` line — including one that literally reads `NO VERDICT, NO THRESHOLD`. Record §8's last line: until two grids agree, ρ is a figure, not a gate |
...
| E8 | **the documented failure** (§10.9) — **this is the rule that produced L4a's order-4 reading** | a synthetic **correctly-wired BW2 pair**: `sign == Sign::Negative` and `rho > 0.5`. Asserted **as the documented failure**, with a comment saying so and naming record §7 **and PR #3 §5**, so a future session that "fixes" ρ into agreeing with the wiring goes red and reads why. Note for the builder: `relativePolarity()` is L4a decision 6b's un-whitened rule finally being written down (`docs/dsp/2026-08-30-sweep-ir-l4a.md:1181` on `main`), and PR #3 showed it is the estimator whose peak-sign reading reproduced the order-4 discrepancy on a band-pass pair. **Shipping it is fine — it answers the same-system question (record §7 table row 3). Letting it answer the across-a-crossover question is not** (H8, H10) |

| E9 | **the L4a geometry, reproduced as a fixture** (PR #3 §4, §5) | two **band-pass** boxes (a sub high-passed at 30 Hz, a main low-passed at 8 kHz) crossed at order 4: `sign` reads **negative** while the order-4 offset at lag 0 is positive, and `lag != 0`. Asserted as a **second documented failure**, with `lag` printed — `lag` leaving 0 is the observable that says the peak stopped tracking the identity. This is the cell PR #3 measured (peak at −3.08 ms, 1.45× the lag-0 value); it belongs in Task F's grid as an expected refusal too (record §8.4) |
...
| H10 | **never a sign off ANY correlation peak, whitened or not**, and an even-order discrepancy is evidence about the SYSTEM (PR #3 §5, §8 at head `cdd8a25`) | on a pair whose band fit reads `agreement < 0.5`, the wizard reports the low `R` as "not a matched pair" and **does not** offer a topology change; `grep -nE 'topolog|family|order' app/src/measure/AlignmentWizard.cpp` shows no write outside the `answer*()` setters (the H1 property, re-asserted where it matters). The rule covers **both** correlators, because PR #3 measured them disagreeing with each other on one unchanged pair: the un-whitened ρ of Task E fails at order 4, the shipped PHAT `inverted` fails at order 8. Neither may reach `verdict()` — which is exactly what H8's invariance asserts for ρ and what H7 confines `inverted` to |
...
| I3 | the marks are marks | `+6.0206 dB` and the topology's designed sum (`0 dB` for LR-N and odd BW-N, `+3.0103 dB` for BW2 inverted) are exposed as reference **marks**. The structural test must match a **declaration**, not the vocabulary a crossover header is obliged to use — the earlier word-grep matched "low-pass", "high-pass" and "look" (verifier defect 5, demonstrated on `ButterworthDesign.h` and `Polarity.h`). Use (GNU grep) `grep -nE '^[[:space:]]*[^/*]*(is|has)?([Pp]ass(ed|es)?|[Ff]ail(ed|s)?|[Aa]ligned|[Oo][Kk])[[:space:]]*[(;=]' app/src/view/CrossoverSurface.h` → **no match**. The leading `[A-Za-z_]*` of the earlier form was the bug (round-2 verifier): it let the word match **inside** a longer identifier, so `double lowPass = 1.0;` and the real `app/src/Main.cpp:119` (`az::ui::AzLookAndFeel lookAndFeel;`) both hit. The identifier must **be** the word, hence `` with no prefix wildcard, plus an explicit `(is|has)?` for `isAligned()`. **Validated on this tree**: it matches `bool passed = false;` and `[[nodiscard]] bool isAligned() const noexcept;`, and returns **nothing** on `app/src/Main.cpp`, `core/include/rta/dsp/ButterworthDesign.h`, `core/include/rta/ir/Polarity.h` and `app/src/view/BodeLayout.h`, nor on prose reading `/// The low-pass side. Have a look.` |
| I4 | **no objective exists** | **AMENDED 2026-09-16 by the Wave 3b build — the word-grep this row specified does NOT catch the mutation this same row prescribes.** It read: `grep -rniE "maximi[sz]e|optimi[sz]e|minimi[sz]e ?(ripple|sum)|argmax.*sum" app/src/view/CrossoverSurface.* app/src/measure/AlignmentWizard.*` → no match. Measured: adding `bestDelayForLoudestSum()` leaves it green — the identifier contains none of those words, and the only line that does is a **comment**, which a declaration-shaped scan strips (D6's amendment) and a raw grep flags inside the shipped header's own twelve-line argument for why the move is forbidden. A word list cannot separate NAMING an objective from ARGUING AGAINST one; this is verifier defect 5's shape again, one row further on. **Shipped instead:** `app/tests/test_crossover_surface.cpp` ENUMERATES the class's member functions from the header and requires the set to match a whitelist exactly, so any objective goes red whatever it is called — `bestDelayForLoudestSum()` fails at `REQUIRE( declared.size() == allowed.size() )`. The I5 half (no member could say "topology inferred") keeps a short word scan, because there the WORDS are the failure mode. Record §6 is unchanged — only this row's mechanism was wrong. |
...
1. **Ratify ALIGN-R11, and tick `HUMAN-QA-QUEUE.md:80`.** The order-4 attribution is **settled** by PR #3 (identity confirmed; the L4a reading attributed to L4a decision 6b's un-whitened peak-sign rule across two different passbands — an estimator this repo does not ship). What is left is a decision, not a measurement: the queue entry says the probe blocks the whole Wave-3 build, and this plan discharges that for the whole lane. One line from the owner closes the entry. **NOT verified by the probe:** the exact band edges of the original L4a fixture (probe §9) — nothing here depends on them.
...
2. **`docs/research/2026-09-15-l7-align-order4-probe.md` §5, §7, §8** (PR #3, pending merge) — the order-4 ruling D5/H9/I1b cite (read PR #3 at head **`cdd8a25`** — §5's attribution was corrected there after its own verifier refuted the first one), the five closed-form cases Task D folds in, and the two bindings it puts on the wizard: **never read a topology sign off ANY correlation peak, whitened or not**, and read an even-order discrepancy as evidence about the system.
