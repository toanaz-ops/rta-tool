...
`docs/reports/008-remote-api.md`.** Both waves are on `origin/main`: Wave 1
(tasks A–G, the pure half) as **PR #16 at `7b4773f`**, Wave 2 (tasks H–K, the
server) as **PR #18 at `91367a8`**. Eleven tasks, ten of them proven in
`RTA_BUILD_APP=OFF` — the only configuration CI runs — including the whole
request path over a real loopback socket. Only the composition-root wiring is
ON, and `api.enabled` ships **`false`**, so nothing an operator can see
changed. Verified by an **independent rebuild at `d071269`** (the merged tree
plus PR #19's comment repair): OFF **774/774**, ON **848/848**, forced fallback
**774/774**, 0 `warning C`, **13/13** guards green in ON and **11/11** in OFF,
8 snapshot PNGs. **Two of the three CI operating systems confirm the OFF number
as well** — see the CI paragraph below, which also says what the third one
found.

**L7 (Solvers) is BUILT and MERGED. That lane is closed out —
...
**station 4 WAVE 0 is BUILT (2026-09-18) and MERGED as PR #17 at `b1e14a9`**
-- branch `l6a/wave0-spl-publish`. (This line said "its PR is open, not
merged" until the L-API closeout on 2026-09-18; PR #17 merged at
2026-09-18T03:36Z, before PR #18 did.) ~~**Waves 1-4 have not started**~~ **CORRECTED 2026-09-26: L6a is BUILT and
CLOSED OUT** — Waves 0-3, 4a, Task G and W2-E all merged; Wave 4b (the served
web viewer) was **CUT by the owner 2026-09-25**. See the L6a row below and
[`docs/reports/009-spl-pro.md`](../reports/009-spl-pro.md). The tallies, design
finding and three falsified tolerances this paragraph originally pointed to
were Wave 0/1's own and are historical; current tallies live in the report.
**Corrected 2026-09-18 at the L-API closeout — this paragraph's own
measurement has rotted.** It used to read "`grep -rn "rta::meter\|rta/meter"`
over the whole repo returns 16 lines, every one under `core/`; `measure::Snapshot`
carries no broadband level, no weighting and no detector." That was true before
PR #17. Re-measured at `d071269`: **111 hits**, across **seventeen** files outside
`core/` carry them — `app/src/measure/Snapshot.h:352` now declares
`std::optional<SplBlockView> spl`, plus `SplConfig`, `SplMeter`, `SplSession`,
`AnalysisPublish` and `AnalysisThreadSpl`. **L6a's first wave built the SPL
publish path and it is merged.**

**And that moved a gate nobody moved with it.** `app/src/api/ApiSerialise.cpp:74-75`
still carries the comment `"spl" joins it the day the Meters track puts SPL in
the Snapshot and not a day earlier`, and `available` is still the hardcoded
six-name literal at `:77`. That day arrived in PR #17, *before* L-API's own
PR #18 merged. The honest reading is not "the list is wrong" — no endpoint
serialises SPL, so advertising `"spl"` would promise a representation nothing
returns — it is that **the stated trigger is stale**: `"spl"` now waits on a
serialiser change in L-API's surface, not on the snapshot. Whoever picks it up
edits one comment and one string literal, and decides whether `"spl"` means a
field on `/snapshot` or an endpoint of its own. Report 008 §7 carries it.
**L8** research lanes
...
PR body and handoff entry that repeated it from 2026-09-17T17:31Z onward.
Measured with `gh run list`: a push to `main` succeeded on all three OSes at
2026-09-17T17:31Z, and every run since has executed. So PRs #16, #17, #18 and
#19 merged with a **live, visibly failing** matrix rather than with none.

What the matrix says at `main` `d071269`: **ubuntu-latest 774/774** and
**windows-latest 774/774** — which is the
first independent confirmation of the OFF tally by anything other than this
machine — and **macos-latest 770/774, 4 failed**:

| failing test | file | whose |
|---|---|---|
...
L-API's own one is diagnosed: the golden body's first divergence is
`-49.341915` emitted against `-49.34192` in the committed golden — **adjacent
float32 values, about one ULP apart**, so the lock is over numbers the DSP
computes and those numbers differ in the last bit between MSVC/x64 and Apple
clang/arm64. D7 is a byte-compare and cannot survive that. It is a **test
portability defect, not a wire-format defect** — `F1`–`F7` (the third-party
...
graph LR
    ...
    P2 --> P6
    P6 --> LAPI
    LAPI -.->|"G7's viewer is a CLIENT<br/>of this one surface"| P6
    P2 --> P7
    P4 --> P7
    L5c --> P7
    P7 --> P9
...
5. ~~Then **L3**~~ — **BUILT 2026-09-06**, see its row above. ~~**L6b** next~~ —
   ...
   **MERGED at `a39a02e`** — it was opened 69 seconds after the L7 closeout's
   ...
   ~~station 4 next~~ — **station 4 DONE and MERGED 2026-09-18: PR #16 at
   `7b4773f` (Wave 1, tasks A–G) and PR #18 at `91367a8` (Wave 2, tasks H–K).
   The lane is CLOSED OUT — `docs/reports/008-remote-api.md`.** **L6a's SPL
   web viewer (G7) rides that surface — it must not open a second one**, and
   the surface is now frozen rather than proposed. ~~Then **L7**, which needed
   L2 + L4 + L5 — two of those three are in.~~ — **L7 BUILT and MERGED
   2026-09-16**, `docs/reports/007-solvers.md`.
6. ~~**Now: L6a (SPL-pro)**~~ — **BUILT and CLOSED OUT 2026-09-26**, see the
   L6a row above and `docs/reports/009-spl-pro.md`. Kept below as history. SPL
   logging / history / alarms / PDF / web viewer
   ...
   **station 3 is DONE 2026-09-17** — plan
   ...
   defaults for the `[!]` questions Q1/Q2/Q8 with their flip costs, and
   ~~**station 4 (build) is next**~~ — **station 4 WAVE 0 is BUILT and MERGED
   2026-09-18 (PR #17 at `b1e14a9`)**; ~~WAVES 1–4 are what is next~~ —
   **WAVES 1-4 are also BUILT and MERGED, 2026-09-26 (Wave 4b cut by the
   owner) — `docs/reports/009-spl-pro.md`.**
   Two corrections this item carried, both now paid:
   - "the Meters track landed… so the dependency is satisfied" was true of
     `core/` and of nothing else — and then Wave 0 fixed it. The
     `grep -rn "rta::meter\|rta/meter"` figure this item printed (**16 lines,
     every one under `core/`**) is **no longer true**: at `d071269` it is
     **111**, across **seventeen** files outside `core/` (one of them
     `app/tests/test_api_serialise.cpp:103`, which is why a count taken from a
     glance at the list came out one short), and
     `app/src/measure/Snapshot.h:352` declares `std::optional<SplBlockView> spl`.
   - "that is also what gates `"spl"` in the remote API's `available` list" is
     **no longer the gate**. The snapshot condition is met; what is left is
     that no serialiser emits SPL and `app/src/api/ApiSerialise.cpp:77` still
     writes the hardcoded six-name literal, with a comment at `:74` naming a
     trigger that has already fired. One comment and one literal, plus the
     decision whether `"spl"` means a field on `/snapshot` or its own endpoint.
   Read `docs/HANDOFF.md`'s top section ("L-API CLOSED OUT"), then its L6a
   Wave 0 entry, then `docs/reports/008-remote-api.md` — Wave 4b is a client of
   that surface and §7 says what is missing from it.
