...
struct SplConfig {
    ...
    /// 0.0 dBFS lands at bin 1200, mid-span. Calibrated with a typical
...
| D1b | one exchange up, half the time | `L = L_c + Q`, `T = T_c/2`, `q = Q/log10(2)`: `D == 100 %` to **1e-12**, for `Q ∈ {3,4,5,6}`. **Not "exactly"**: the term is `10^(Q·log10(2)/Q)`, and `std::pow(10.0, std::log10(2.0))` is not bit-identically `2.0`. Only D1a, whose exponent is `0`, is exact |
| D1c | one exchange down, twice the time | `L = L_c − Q`, `T = 2·T_c`, same `q`: `D == 100 %` to **1e-12**, same reason |
...
| `measure_has_no_framework_deps` | the hand-appended entries **sum to 19** across the waves — W0-B0 **+1** (`AllocationProbe.h`, picked up free by the trailing `*.h`), W0-B **+3**, W0-C **+0**, W0-D **+0** (both `AnalysisThread` files use JUCE and must never be listed), W1-E **+1**, W2-A **+2**, W2-B **+2**, W2-C **+2**, W2-D **+1**, W3-A **+2**, W4a **+4**, plus one per new test **header** only. **Recompute from the list at `main`; `.cpp` files in `app/tests/` are never globbed** | misspelling one entry — the failure mode `memory/core-must-not-include-frameworks.md` records, where coverage shrinks **silently**. The proof it did not is the risen `OK (N files scanned)` count, read from the output |
