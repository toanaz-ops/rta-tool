...
**The wizard must keep §3's table unchanged and must not read a sign off a
cross-correlation peak at all.**
...
other. The nearest cross-correlation in `core/` is `DelayFinder.cpp:34`
(`result.inverted = peak.height < 0.0`), and its only test negates the *same*
pink-noise signal (`core/tests/test_delay_finder.cpp:50-58`).
...
The L4a reading is the **sign of the cross-correlation peak** between two
impulse responses, both driven with the same sign. A negative peak is the
"wrong sign". Five geometries, orders 1/2/3/4/6/8, 1 s impulse responses at
48 kHz. `rho = |peak| / √(E₁E₂)`, the bounded figure L4a decision 6b settled on.
...
- **G3 and G4 reproduce the L4a report exactly: right at 1, WRONG at 2, WRONG at
  4, right at 8.** Both are pairs of band-pass boxes — the box model
  `tools/probe_polarity_bandwidth.py:132-152` builds and the only one this repo
  has committed.
- G5 (one complementary low-pass against a band-passed main) does **not**
  reproduce it: it reads right at 4. So it is the **sub** being band-passed
  below, not the main being band-passed above, that does the damage — which
  matters, because a real subwoofer is a band-pass and the wizard will meet one.
...
2. **The correlation peak leaves lag 0.** The identity constrains only the value
   ...
   **−3.08 ms** and is **1.45×** larger than the value at lag 0, and adjacent
   ...
   lobe's sign. At order 8 the peak moves further still (−6.33 ms) and happens
   to land on a positive lobe again, which is why L4a saw the sign "flip back".

**So the L4a order-4 reading is a fixture artefact of the peak-sign rule applied
across two different passbands, not a counter-example to the identity.** L4a's
...
| The rule that produces it | `core/src/dsp/DelayFinder.cpp:34` — `result.inverted = peak.height < 0.0` |
...
phase. Restored, rebuilt from the current tree with the exe deleted first:
...
applied a cross-correlation **peak-sign** rule across two systems with different
passbands: a band-pass sub against a band-pass main spreads the HP−LP offset by
62.9° across the overlap instead of holding it constant, which stretches the
correlation envelope until a neighbouring, oppositely-signed lobe outgrows the
value at lag 0 (peak at −3.08 ms, 1.45× the lag-0 value) — while the value *at
lag 0* stays positive, exactly as the identity says. No sign convention in this
engine can produce that reading: conjugating the transfer function negates the
offset, and −0° = 0° and −180° = 180°, so an even-order pair reads identically
either way. **The wizard must therefore (a) keep §3's table, (b) never read a
topology sign off a cross-correlation peak — the §4 complex band fit with its
bounded agreement `R` is the estimator, and `R` collapsing is precisely how it
reports "these two are not a matched pair", and (c) treat an even-order sign
discrepancy as evidence about the *system* (a band-passed source, a processor
whose high-pass is `(−s)^N/D`, or a wiring inversion the operator must be asked
about under question (c)), never as evidence about a convention.**
...
- §13.1's order-4 attribution. The identity is confirmed; the L4a reading is
  attributed to the fixture's geometry and the peak-sign rule.
