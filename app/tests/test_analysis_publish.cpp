...
TEST_CASE("fillSplPublishScalars copies every field it owns, independently",
    ...
    rta::measure::fillSplPublishScalars(input, 3, 5, true, &channelState, 7, true, true);
    ...
    CHECK(input.logWriteFailed == true);
    CHECK(input.calibrationInvalid == true);

    // The FALSE/zero/nullptr side too, so a mutation hardcoding any one field
    // to a fixed true/nonzero value is caught.
    rta::measure::SplPublishInput cleared;
    rta::measure::fillSplPublishScalars(cleared, 0, 0, false, nullptr, 0, false, false);
    ...
    CHECK_FALSE(cleared.logWriteFailed);
