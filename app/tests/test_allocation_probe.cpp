...
TEST_CASE("AllocationProbe only charges the thread that armed it",
    ...
    // thread. `backgroundSink` is local and touched ONLY by this thread
    // (never `g_sink`, which the main thread's own second probe below
    // writes), so there is no data race between the two threads' escapes.
    ...
    INFO("bytes charged to an IDLE arming thread while a background thread "
        << "allocated concurrently = " << bytesWhileIdle);
