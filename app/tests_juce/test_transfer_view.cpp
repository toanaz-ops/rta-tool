...
// CATCHES: the final-review finding -- `absoluteColumnsForBins` accepting a
// column left of the plot's own left edge. `PlotGeometry::xForHz` is not
// clamped below `fLowHz` (PlotGeometry.h's own contract comment: a caller
// can ask "what's past the edge"), so a bin between `hzForX(0)` and 20 Hz
// maps to a column inside the 40px level-label gutter, and `bridgeGaps`
// amplifies that one stray bin into a bridged run across the whole gutter.
// Reproduced at the exact width the reviewer found it at: 500px wide, the
// app's default fftSize (4096, binHz = 48000/4096 = 11.71875 Hz), where bin 1
// (11.72 Hz) maps to column 6 -- inside `axis.left`'s floor of 40. At
// 1100x760 there is no bin between 0 Hz and fLowHz close enough to the axis
// to land off it, which is why the defect survived the lane at that size.
TEST_CASE("the live trace never paints into the level-label gutter", "[transfer-view]") {
    const auto snap = snapshotWithTransfer(4096, 48000.0, 10);
    const StaticSnapshotSource source(snap);
    TransferView view(source);
    view.setSize(500, 760);
    view.resized();

    const auto image = renderView(view, 500, 760);

    const FrequencyAxis axis = rta::view::frequencyAxis(PaneRect{ 0, 0, 500, 760 });
    // The gutter is everything strictly left of the axis's own left edge --
    // exactly the region `absoluteColumnsForBins`' floor is meant to protect.
    const int gutterRight = static_cast<int>(axis.left);

    const PlotGeometry magnitudeGeometry =
        rta::view::paneGeometry(axis, view.panes().magnitude, 18.0, -18.0);
    const PlotGeometry phaseGeometry = rta::view::paneGeometry(axis, view.panes().phase, 180.0, -180.0);

    juce::Rectangle<int> magnitudeGutter(0, static_cast<int>(magnitudeGeometry.top), gutterRight,
                                         static_cast<int>(magnitudeGeometry.bottom - magnitudeGeometry.top));
    juce::Rectangle<int> phaseGutter(0, static_cast<int>(phaseGeometry.top), gutterRight,
                                     static_cast<int>(phaseGeometry.bottom - phaseGeometry.top));

    CHECK_FALSE(anyPixelMatches(image, magnitudeGutter, isLiveTraceish));
    CHECK_FALSE(anyPixelMatches(image, phaseGutter, isLiveTraceish));
}
